...
def dTdt(t,y):
    ...
    if (y[1] >= 113.15):
        ...
        dT2dt = (B3 * (Tw - Tg) - B4 * (Tg - Tf)  - (1.5922*1005*(Tg - To)) - 812056) / C2
    else:
...
if below_115.size > 0:
    t_below_115 = (sol.t[below_115[0]])/60
    times_below_115[hf] = t_below_115
    text1 = fr"$h_{{f}} = {hf}$ W/m$^2$, $t_{{115}}$ = {t_below_115:.0f} min ,$T_{{lowest}}$ = {(np.min(sol.y[0])-273):.0f} $^oC$"  

else:
    times_below_115[hf] = None
    text1 = fr"$h_{{f}} = {hf}$ W/m$^2$, $t_{{115}}$ = N/A ,$T_{{lowest}}$ = {(np.min(sol.y[0])-273):.0f} $^oC$" 

axes[1,0].plot((sol.t)/60, sol.y[0], label=fr"Tw (wall)")
axes[1,0].plot((sol.t)/60, sol.y[1], label="Tg (gas)")

axes[1,0].text(0.40, 0.8, 
         text1,
...
axes[1,0].text(0.4, 0.7,
    "• Assumptions: \n"
    "• 1:  80% Volume occupied by gas 20% Occupied by spray\n"
    "• 2:  Mass of spray entering = Mass of air leaving\n"
    "• 3:  100% Spray evaporation until Tg>=160 degree Celcius\n",
        transform=axes[1,0].transAxes,
         fontsize=10,
...
def dTdt(t,y):
    ...
    if(y[2] >= 113.15):
        ...
        dT3dt = (B5 * (Tw - Tg) - B6 * (Tg - Tf)  - (1.5922*1005*(Tg - To)) - 821056) / Cg
    else:
...
if below_115.size > 0:
    t_below_115 = (sol.t[below_115[0]])/60
    times_below_115[hf] = t_below_115
    text1 = fr"$h_{{f}} = {hf}$ W/m$^2$, $t_{{115}}$ = {t_below_115:.0f} min, $T_{{lowest}}$ = {(np.min(sol.y[1])-273):.0f} $^oC$"

else:
    times_below_115[hf] = None
    text1 = fr"$h_{{f}} = {hf}$ W/m$^2$, $t_{{115}}$ = N/A, $T_{{lowest}}$ = {(np.min(sol.y[1])-273):.0f} $^oC$"
...
plt.tight_layout()
plt.savefig("Comparison_of_different_models_t1.png",dpi=300)
