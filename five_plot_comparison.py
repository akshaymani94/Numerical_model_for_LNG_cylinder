...
def dTdt(t,y):
    ...
    dT3dt = (B5 * (Tw - Tg) - B6 * (Tg - Tf)  - (0.46721*1005*(Tg - To)) - 95.2) / Cg
...
for idx,hf in enumerate(hf_values):
    ...
    axes[0,2].plot((sol.t)/60, (sol.y[0]), label="Tin (insulation) " + text1, linestyle=':', linewidth=2.5, color=colors[idx] )
    axes[0,2].plot((sol.t)/60, sol.y[1], label=fr"Tw (wall)", linestyle='-', linewidth=1, color=colors[idx])
...
axes[0,2].set_title("Temperature Change Over Time for Two-node Model with Insulator Layer")
axes[0,2].set_ylim(90,330)
axes[0,2].set_yticks(np.arange(90,331,20))
...
plt.tight_layout()
plt.savefig("Five_plot_subplot.png",dpi=300)
